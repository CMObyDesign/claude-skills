...
# ─── CUSTOM OBJECTS (Children, etc.) ─────────────────────────
@mcp.tool()
def list_objects(location_id: str) -> dict:
    """List all custom object schemas in a subaccount (e.g. a 'Children' object)."""
    r = requests.get(f"{GHL_BASE_URL}/objects/", headers=ghl_headers(location_id), params={"locationId": location_id})
    return r.json()

@mcp.tool()
def get_object_schema(location_id: str, object_key: str) -> dict:
    """Get a custom object schema and its fields. object_key e.g. 'custom_objects.children'."""
    r = requests.get(f"{GHL_BASE_URL}/objects/{object_key}", headers=ghl_headers(location_id), params={"locationId": location_id, "fetchProperties": "true"})
    return r.json()

@mcp.tool()
def create_object_field(location_id: str, object_key: str, name: str, dataType: str = "TEXT") -> dict:
    """Create a custom field on a custom object (dataType: TEXT, LARGE_TEXT, NUMERICAL, PHONE, DATE, SINGLE_OPTIONS, MULTIPLE_OPTIONS, CHECKBOX, RADIO)."""
    payload = {"locationId": location_id, "name": name, "dataType": dataType, "objectKey": object_key, "showInForms": True}
    r = requests.post(f"{GHL_BASE_URL}/custom-fields/", headers=ghl_headers(location_id), json=payload)
    return r.json()

@mcp.tool()
def search_object_records(location_id: str, object_key: str, query: str = "") -> dict:
    """Search records in a custom object by text query."""
    payload = {"locationId": location_id, "page": 1, "pageLimit": 20, "query": query}
    r = requests.post(f"{GHL_BASE_URL}/objects/{object_key}/records/search", headers=ghl_headers(location_id), json=payload)
    return r.json()

@mcp.tool()
def create_object_record(location_id: str, object_key: str, properties_json: str) -> dict:
    """Create a record in a custom object. properties_json = JSON string of field->value, e.g. '{"first_name":"Mia","procare_student_id":"123"}'."""
    try:
        props = json.loads(properties_json)
    except Exception as e:
        return {"error": f"invalid properties_json: {e}"}
    payload = {"locationId": location_id, "properties": props}
    r = requests.post(f"{GHL_BASE_URL}/objects/{object_key}/records", headers=ghl_headers(location_id), json=payload)
    return r.json()

@mcp.tool()
def upsert_object_record(location_id: str, object_key: str, unique_field: str, unique_value: str, properties_json: str) -> dict:
    """Create OR update a record, deduped by unique_field==unique_value (prevents duplicate children). properties_json = JSON of all fields to set. Use procare_student_id as unique_field for children."""
    try:
        props = json.loads(properties_json)
    except Exception as e:
        return {"error": f"invalid properties_json: {e}"}
    props[unique_field] = unique_value
    existing_id = None
    try:
        s = requests.post(f"{GHL_BASE_URL}/objects/{object_key}/records/search", headers=ghl_headers(location_id),
                          json={"locationId": location_id, "page": 1, "pageLimit": 20, "query": str(unique_value)})
        for rec in s.json().get("records", []):
            rp = (rec.get("properties") or {})
            if str(rp.get(unique_field)) == str(unique_value):
                existing_id = rec.get("id"); break
    except Exception:
        pass
    if existing_id:
        r = requests.put(f"{GHL_BASE_URL}/objects/{object_key}/records/{existing_id}", headers=ghl_headers(location_id),
                         json={"locationId": location_id, "properties": props})
        out = r.json() if r.content else {}
        return {"_action": "updated", "_record_id": existing_id, "result": out}
    r = requests.post(f"{GHL_BASE_URL}/objects/{object_key}/records", headers=ghl_headers(location_id),
                      json={"locationId": location_id, "properties": props})
    out = r.json() if r.content else {}
    return {"_action": "created", "result": out}

@mcp.tool()
def get_associations(location_id: str) -> dict:
    """List association schemas (relationships between objects, e.g. Contact<->Children)."""
    r = requests.get(f"{GHL_BASE_URL}/associations/", headers=ghl_headers(location_id), params={"locationId": location_id})
    return r.json()

@mcp.tool()
def associate_records(location_id: str, association_id: str, first_record_id: str, second_record_id: str) -> dict:
    """Link two records via an association (e.g. a parent Contact to a Child record)."""
    payload = {"locationId": location_id, "associationId": association_id, "firstRecordId": first_record_id, "secondRecordId": second_record_id}
    r = requests.post(f"{GHL_BASE_URL}/associations/relations", headers=ghl_headers(location_id), json=payload)
    return r.json()
