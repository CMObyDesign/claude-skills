...
def get_funnels(location_id: str) -> dict:
    ...
    r = requests.get(f"{GHL_BASE_URL}/funnels/", headers=ghl_headers(location_id), params={"locationId": location_id})
    return r.json()
...
def create_funnel_page(location_id: str, funnel_id: str, name: str, html: str) -> dict:
    ...
    r = requests.post(f"{GHL_BASE_URL}/funnels/{funnel_id}/pages", headers=ghl_headers(location_id), json=payload)
    return r.json()
